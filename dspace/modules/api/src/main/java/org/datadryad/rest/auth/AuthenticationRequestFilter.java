...
public class AuthenticationRequestFilter implements ContainerRequestFilter {
    ...
    public ContainerRequest filter(ContainerRequest containerRequest) {
        ...
        try {
            ...
            log.error("OAuth problem");
        } catch (OAuthSystemException ex) {
            AuthHelper.throwExceptionResponse(ex, Status.INTERNAL_SERVER_ERROR, "OAuth System Exception");
        } catch (NullPointerException e) {
            // This can legitimately happen if an access token isn't provided in either the query or the header
            log.error("No OAuth in query or header");
        }
